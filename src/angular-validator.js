...
validator.service('hiValidatorService', ['$templateCache', function($templateCache) {
  ...
  var _validators = {},
    defaultTemplateKey = 'hiValidatorErrorTemplate';
  ...
  var _initialize = function() {
    if (!$templateCache.get(defaultTemplateKey)) {
      $templateCache.put('hiValidatorErrorTemplate',
          '<div ng-repeat="(key, value) in errors" class="text-danger">' +
            '<span ng-bind="value.errorMessage"></span>' +
          '</div>');
    }
  };
...
validator.directive('hiValidate', ['hiValidatorService', '$log', function(hiValidatorService, $log) {
  /**
...
validator.directive('hiValidatorMessage', function() {
  ...
  return {
    ...
    templateUrl: function(elem, attrs) {return attrs.tpl || 'hiValidatorErrorTemplate';},
